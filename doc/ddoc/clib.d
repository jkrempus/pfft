...
to refer the length of the input sequence in the function descriptions below).
The second argument is optional. If null is passed as mem, the function will
...
/**
This function returns the size of a memory block needed by pfft_table_f. See
the description of pfft_table_f above. 
 */
...
to pfft_table_f when creating table. The method operates in place - the 
...
is to use pfft_allocate_f. If you want to take care of memory allocation in
...
/**
This function is an inverse of pfft_fft_f, scaled by n. See the
description of pfft_fft_f.
 */
...
/**
This function is used in the same way as pfft_table_f, the only difference is
that it returns an instance of struct PfftRTableF.
 */
extern(C) PfftRTableF pfft_rtable_f(size_t n, void* mem);

/**
This function returns the size of a memory block needed by pfft_rtable_f.
See the descriptions of pfft_rtable_f and pfft_table_f.
 */
extern(C) size_t pfft_rtable_size_bytes_f(size_t n);

/**
This function is used in the same was as pfft_table_free_f, the only difference
is that it takes an instance of struct PfftRTableF as a parameter.
 */
...
make sure that the addresses re and im are multiples of the number returned by 
pfft_alignment_f.
 */
...
$(I DFT(f)[i] = adj(DFT(f)[n - i])). When the method completes, the array 
contains the real part of the inverse discrete fourier transform of the 
input sequence, scaled by n. The imaginary part is known to be equal to 0.
